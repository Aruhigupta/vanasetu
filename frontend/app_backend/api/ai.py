...
from app_backend.schemas.schemas import AIHerbImageCheck, AIQualityPredictionInput
from app_backend.services.ai_service import HerbAIService

router = APIRouter(prefix="/ai", tags=["AI Engine Services"])

@router.post("/detect-fake-herb")
def detect_fake_herb(req: AIHerbImageCheck):
    return HerbAIService.detect_fake_herb_image(req.image_url_or_hash, req.claimed_herb_name)
...
def detect_anomalies(batch_id: str):
    return HerbAIService.detect_supply_chain_anomalies([{"batch_id": batch_id}])

@router.get("/generate-quality-report/{batch_id}")
def generate_quality_report(batch_id: str, herb_name: str = "Ashwagandha"):
    return HerbAIService.generate_ayush_quality_report(batch_id, herb_name, {
